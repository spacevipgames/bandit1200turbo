...
static void test_corrections_TAE_map_weight_zero_uses_tps_only()
{
  ...
  TEST_ASSERT_EQUAL((100+132), accelValue);
	TEST_ASSERT_BIT_HIGH(BIT_ENGINE_ACC, currentStatus.engine);
	TEST_ASSERT_BIT_LOW(BIT_ENGINE_DCC, currentStatus.engine);
}
...
static void test_corrections_TAE_map_weight_blends_mapdot()
{
  ...
  TEST_ASSERT_EQUAL(100 + table2D_getValue(&taeTable, currentStatus.tpsDOT / 10), accelValue);
	TEST_ASSERT_BIT_HIGH(BIT_ENGINE_ACC, currentStatus.engine);
	TEST_ASSERT_BIT_LOW(BIT_ENGINE_DCC, currentStatus.engine);
}
