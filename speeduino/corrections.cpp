...
void initialiseCorrections(void)
{
  ...
  transientsDFCOReentryReset();
  transientsSpoolAeReset();
}
...
uint16_t correctionsFuel(void)
{
  ...
  byte dfcoReentryCorrection = transientsDFCOReentryFuelMult();
  if (dfcoReentryCorrection != 100) { sumCorrections = div100(sumCorrections * dfcoReentryCorrection); }
...
uint16_t correctionAccel(void)
{
  ...
  else if(configPage2.aeMode == AE_MODE_TPS)
  {
    ...
    if (configPage2.aeMapWeight > 0U)
    {
      ...
      currentStatus.tpsDOT =
        (((int32_t)currentStatus.tpsDOT * (100 - configPage2.aeMapWeight)) +
         ((int32_t)currentStatus.mapDOT * configPage2.aeMapWeight)) / 100;
    }
  }
  ...
  accelValue = (int16_t)transientsSpoolAeOverlay((uint16_t)accelValue);
  return accelValue;
}
...
byte correctionDFCOfuel(void)
{
  ...
  if ( BIT_CHECK(currentStatus.status1, BIT_STATUS1_DFCO) )
  {
    ...
    {
      const uint8_t taperTime = configPage9.dfcoTaperTime;
      if (taperTime > 0U)
      {
        //Do a check if the user reduced the duration while active to avoid overflow
        if (dfcoTaper > taperTime) { dfcoTaper = taperTime; }
        scaleValue = map(dfcoTaper, taperTime, 0, 100, configPage9.dfcoTaperFuel);
        if( BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ) ) { dfcoTaper--; }
      }
      else
      {
        //Guard against division by zero inside map() when taper time is set to 0.
        scaleValue = configPage9.dfcoTaperFuel;
        dfcoTaper = 0;
      }
    }
...
bool correctionDFCO(void)
{
  ...
  transientsDFCOReentryUpdate(DFCOValue);
  if (DFCOValue && transientsDFCOReentryReleaseRequest())
  {
    DFCOValue = false;
    dfcoDelay = 0;
  }
...
int8_t correctionDFCOignition(int8_t advance)
{
  ...
  if ( (configPage9.dfcoTaperEnable == 1) && BIT_CHECK(currentStatus.status1, BIT_STATUS1_DFCO) )
  {
    ...
    {
      dfcoRetard -= map(dfcoTaper, taperTime, 0, 0, configPage9.dfcoTaperAdvance);
    }
