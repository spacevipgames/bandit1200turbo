...
void __attribute__((always_inline)) loop(void)
{
      ...
      #if defined(secondarySerial_AVAILABLE) && !MOTO_PROFILE_DISABLE_SECONDARY_SERIAL
      ...
      #endif
      #if defined (NATIVE_CAN_AVAILABLE) && !MOTO_PROFILE_DISABLE_CAN
    ...
    if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_4HZ))
    {
      ...
      {
        #if !MOTO_PROFILE_DISABLE_SECONDARY_SERIAL && !MOTO_PROFILE_DISABLE_CAN
...
uint16_t PW(int REQ_FUEL, byte VE, long MAP, uint16_t corrections, int injOpen)
{
  ...
  if (intermediate != 0)
  {
    ...
    if ( BIT_CHECK(currentStatus.engine, BIT_ENGINE_ACC) || transientsSpoolAeActive() )
    {
      //AE Adds % of req_fuel
